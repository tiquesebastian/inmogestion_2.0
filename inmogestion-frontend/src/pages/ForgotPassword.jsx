...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:4000/api/password-recovery/forgot-password-usuario', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.token) {
          setMessage(`${data.message}\n\n🔗 Modo desarrollo: Abre la consola del navegador (F12) para ver el enlace de recuperación.`);
          console.log('🔑 Token de recuperación:', data.token);
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-lg">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm">
              <Link to="/forgot-password-cliente" className="font-medium text-green-600 hover:text-green-500">
                ¿Eres cliente? →
              </Link>
            </div>
          </div>
